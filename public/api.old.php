...
$app->group('/api',function () use ($app, $log) {
        // Common to all sub routes Get contacts
        $app->get('/', function () {
            echo "<h1>This can be the documentation entry point</h1>";
            echo "<p>This URL could also contain discovery nformation in side the headers</p>";
        });
        ...
        $app->group('/v1', function () use ($app, $log) {
                ...
                $app->get('/contacts', function () use ($app, $log) {
                        ...
                        if (!empty($filters)) {
                            foreach ($filters as $key => $value) {
                                if ('q' == $key) {
                                    $results->whereRaw('(`firstname` LIKE ? OR `email` LIKE ?)',
                                        array('%'.$value.'%', '%'.$value.'%')
                                    );
                                } else {
                        ...
                        #$app->view()->appendData(array('data' => $contacts));
                        #$app->view()->fetch('contacts.html');
                        #$twig = $app->view()->getEnvironment();
                        #$template = $twig->render('contacts.html');
                        #echo $template->render(array('data' => $contacts));
                        ...
                        #$app->render('contacts.html',array('contacts' => $contacts));
                ...
                $app->get('/contacts/:id/notes', function ($id) use ($app, $log) {
                        ...
                        if ($contact) {
                            $notes = \ORM::forTable('notes')->where('contact_id', $id)->findArray();
