    ...
    public function call(){
        ...
        if (preg_match('|^' . $this->root . '.*|',$this->app->request->getResourceUri() )) {
            ...
            if ($key = $this->app->user['apikey']) {
                $data = $this->fetch($key);
                if (false === $data) {
                    // First time or previous perion expired,initialize and save a new entry
                ...
                // Set rating headers
                $response->headers->set('X-Rate-Limit-Limit',$this->max);
                $response->headers->set('X-Rate-Limit-Reset',$reset);
